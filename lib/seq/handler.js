...
repo.one = async function (name, ctx, { ext, routeHooks }) {
  try {
    ...
    cond.where = await routeHooks.one.cond(cond.where, {name})
    if (project.length > 0) {
...
repo.list = async function (name, ctx, { ext, routeHooks }) {
  try {
    ...
    match.where = await routeHooks.list.cond(match.where, {name})
    if (project.length > 0) {
...
repo.delete = async function (name, ctx, { ext, routeHooks }) {
  try {
    ...
    for (const item of docs) {
      const update = await routeHooks.delete.update({ id: item.id }, {name})
      const result = await Model.update(update, { where: { id: item.id }, fields: Object.keys(update) })
      results.push(result)
    }
    ctx.status = 200
    ctx.body = results

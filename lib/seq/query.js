...
function Query ({ cond = '%7B%7D', sort = '-_createdAt', project = '', preload = '', page = 1, size = 20, range = 'PAGE' }) {
  if (!(this instanceof Query)) {
    return new Query({ cond, sort, project, preload, page, size, range })
  }
  try {
    this.query = { cond, sort, project, preload, page, size, range }
    this.cond = {}
    this.operators = {}
    this.sort = {}
    this.project = []
...
Query.prototype.buildPreload = function (model) {
  if (this.query.preload && this.query.preload.trim()) {
    ...
    this.preload = this.query.preload
      .trim()
      .split(',')
      .filter(x => !!x)
      .map(x => x.trim())

...
const _ = require('lodash'); // const logger = require('../logger')
...
function () {
  var _ref3 = _asyncToGenerator(function* (name, ctx, ext, opts) {
    ...
    ctx.status = 200;
    const ret = yield Model.create(form.docs);
    ctx.body = ret;
  });
...
function () {
  var _ref4 = _asyncToGenerator(function* (name, ctx, ext, opts) {
    ...
    const ret = yield Model.bulkWrite(docs);
    ctx.status = 200;
    ctx.body = ret;
  });

...
require(['jquery', 'lodash', 'locales', 'handlebarsExtended', './api_project.js', './api_data_sys.js', './api_data.js', 'prettify', 'utilsSampleRequest', 'semver', 'webfontloader', 'bootstrap', 'pathToRegexp', 'list'], function ($, _, locale, Handlebars, apiProject, apiData, apiDataSys, prettyPrint, sampleRequest, semver, WebFont) {
  // load google web fonts
  loadGoogleFontCss();
  var api = apiData.api.concat(apiDataSys.api); //
  // Templates
  //
  ...
  if (apiProject.template.forceLanguage) locale.setLanguage(apiProject.template.forceLanguage); // Setup jQuery Ajax
  ...
  $.each(apiByGroupAndName, function (index, groupEntries) {
    ...
    titles.sort(); // custom order

    if (apiProject.order) titles = sortByOrder(titles, apiProject.order, '#~#'); // add single elements to the new list
  ...
  apiGroups.sort(); // custom order

  if (apiProject.order) apiGroups = sortByOrder(apiGroups, apiProject.order); // sort versions DESC
  ...
  $(document).attr('title', title); // remove loader

  $('#loader').remove(); // render sidenav

  var fields = {
    nav: nav
  };
  $('#sidenav').append(templateSidenav(fields)); // render Generator

  $('#generator').append(templateGenerator(apiProject)); // render Project

  _.extend(apiProject, {
    versions: apiVersions
  });

  $('#project').append(templateProject(apiProject)); // render apiDoc, header/footer documentation
  ...
  apiGroups.forEach(function (groupEntry) {
    ...
    var description = '';
    articleVersions[groupEntry] = {}; // render all articles of a group
    ...
    api.forEach(function (entry) {
      if (groupEntry === entry.group) {
        ...
        if (entry.groupTitle) title = entry.groupTitle; // TODO: make groupDescription compareable with older versions (not important for the moment)
  ...
  $('#sections').append(content); // Bootstrap Scrollspy
  ...
  $('.sidenav').find('a').on('click', function (e) {
    ...
    if ($(id).length > 0) $('html,body').animate({
  ...
  function initDynamic() {
    ...
    $('.nav-tabs-examples').find('a:first').tab('show'); // sample request switch
    ...
    $(window).scrollspy('refresh'); // init modules

    sampleRequest.initDynamic();
  }

  initDynamic(); // Pre- / Code-Format
  ...
  $('#versions li.version a').on('click', function (e) {
    ...
    $('#version strong').html(selectedVersion); // hide all

    $('article').addClass('hide');
    $('#sidenav li:not(.nav-fixed)').addClass('hide'); // show 1st equal or lower Version of each entry
    ...
    $('article[data-version]').each(function (index) {
      ...
      if (semver.lte(version, selectedVersion)) {
        if ($('article[data-group=\'' + group + '\'][data-name=\'' + name + '\']:visible').length === 0) {
          // enable Article
          $('article[data-group=\'' + group + '\'][data-name=\'' + name + '\'][data-version=\'' + version + '\']').removeClass('hide'); // enable Navigation
  ...
  $('#compareAllWithPredecessor').on('click', changeAllVersionCompareTo); // change version of an article

  $('article .versions li.version a').on('click', changeVersionCompareTo); // compare url-parameter
  ...
  function changeVersionCompareTo(e) {
    ...
    if (compareVersion && articleVersions[group][name][0] === selectedVersion || version === selectedVersion) {
      ...
      $root.after(content);
      var $content = $root.next(); // Event on.click re-assign

      $content.find('.versions li.version a').on('click', changeVersionCompareTo); // select navigation

      $('#sidenav li[data-group=\'' + group + '\'][data-name=\'' + name + '\'][data-version=\'' + currentVersion + '\']').addClass('has-modifications');
      $root.remove(); // TODO: on change main version or select the highest version re-render
    }
  ...
  function resetArticle(group, name, version) {
    ...
    $root.after(content);
    var $content = $root.next(); // Event on.click muss neu zugewiesen werden (sollte eigentlich mit on automatisch funktionieren... sollte)
